...
const genealogyController = {
  ...
  addMemberUnderTree: (req, res) => {
    ...
    connection.query(countDescendantsQuery, [parentMemberId], (countErr, countResult) => {
      if (countErr) {
        ...
        if (descendantCount < 10) {
          ...
          connection.query(
            ...
            (insertErr, insertResult) => {
              if (insertErr) {
                ...
                connection.query(insertTreeQuery, [parentMemberId, newMemberId], (treeInsertErr, treeInsertResult) => {
                  if (treeInsertErr) {
                    console.error("Error adding member to genealogy tree:", treeInsertErr.message);
                    res.status(500).json({ error: "Internal Server Error" });
                  } else {
                    res.status(200).json({ message: "Member and relationship added successfully", treeInsertResult });
                  }
                });
              }
            }
          ...
          res.status(400).json({ error: "Genealogy tree limit reached for the parent member." });
        }
      }
    });
  }
